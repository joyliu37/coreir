...
Namespace* CoreIRLoadLibrary_float_CW(Context* c) {
  ...
  auto mul_tg = fpcw->newTypeGen(
    ...
    [](Context* c, Values args) {
      uint exp_bits = args.at("exp_bits")->get<int>();
      uint frac_bits = args.at("frac_bits")->get<int>();
      ASSERT(frac_bits >=10,"Cannot instantiate multiplier less than 10 bits");
      uint width = 1+exp_bits+frac_bits;
      Type* ptype = c->Bit()->Arr(width);
      return c->Record({
        {"a",c->Flip(ptype)},
  ...
  fp->getGenerator("add")->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
    ...
    auto add = def->addInstance("mi","float_CW.add",{{"exp_bits",args.at("exp_bits")},{"frac_bits",args.at("frac_bits")},{"ieee_compliance",Const::make(c,false)}});
...
/*
  fp->getGenerator("mul")->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
    ...
    if (frac_bits ==7 && exp_bits ==8) { //Special case BFloat16 for now
      auto mul = def->addInstance("mi","float_CW",{{"exp_bits",args.at("exp_bits")},{"frac_bits",Const::make(c,10)},{"ieee_compliance",Const::make(c,false)}});
      ...
      auto round_pred = C.or_(C.and_(resx2,C.or_(resx1,resx0)),C.and_(int_out->sel(0),resx2));
      ...
      auto mul = def->addInstance("mi","float_CW.mul",{{"exp_bits",args.at("exp_bits")},{"frac_bits",args.at("frac_bits")},{"ieee_compliance",Const::make(c,false)}});
